...
select t.*
  from tbl_menu t
 order
    by t.category_code asc
      ,t.menu_price desc;

-- 6번. tbl_menu 테이블에서 주문가능상태(orderable_status) 기준으로 정렬하되, 'Y'가 먼저 나오도록 조회하세요.
select t.orderable_status status
  from tbl_menu t
 order
    by case
       when t.orderable_status = 'Y' then 1
       when t.orderable_status = 'N' then 2
       else 3
       end;
...
select t.menu_code code
      ,t.menu_name name
      ,t.menu_price price
  from tbl_menu t
 where t.menu_price >= '15000';


-- 10번. tbl_menu 테이블에서 카테고리 코드가 8번이 아닌 메뉴들을 조회하세요.
select t.menu_name name
  from tbl_menu t
 where t.category_code <> '8';
...
select t.menu_name name
  from tbl_menu t
 where t.menu_price >= '5000'
   and t.orderable_status = 'Y';

-- 12번. tbl_menu 테이블에서 카테고리 코드가 4번이거나 가격이 9,000원인 메뉴를 조회하세요.
select t.menu_name name
      ,t.category_code ct_code
      ,t.menu_price price
  from tbl_menu t
 where t.category_code = '4'
    or t.menu_price = '9000';
...
select t.menu_name name
     , t.menu_price price
  from tbl_menu t
 where t.menu_price >= '8000'
   and t.menu_price <= '20000'
 order
    by t.menu_price;
...
select t.menu_code code
      ,t.menu_name name
  from tbl_menu t
 where t.menu_code between '5' and '15'
 order
    by t.menu_code;
...
select t.menu_name name
      ,t.menu_price price
  from tbl_menu t
 where t.menu_price not between '10000' and '20000'
 order
    by t.menu_price;
...
select t.category_code ct_code
      ,t.menu_name name
  from tbl_menu t
 where t.category_code in ('4','8','12')
 order
    by t.category_code;


-- 23번. tbl_category 테이블에서 카테고리 코드가 1, 2, 3번이 아닌 카테고리를 조회하세요. (NOT IN 사용)
select c.category_code ct_code
      ,c.category_name ct_name
  from tbl_category c
 where c.category_code not in ('1','2','3')
 order
    by c.category_code;
...
select c.ref_category_code rf_code
      ,c.category_name ct_name
  from tbl_category c
...
select t.menu_name name
      ,t.menu_price price
  from tbl_menu t
 where t.menu_name like '%아%'
    or t.menu_price <= '5000'
 order
    by t.menu_price;
